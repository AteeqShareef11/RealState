...
import { getAllCustomers } from '../Redux/slice/buyer';
...
const TABLE_HEAD = [
  ...
  { id: 'monthlyInstallment', label: 'Monthly Installment', alignRight: false },
  { id: 'startDate', label: 'Start Date', alignRight: false },
  { id: 'totalAmount', label: 'Total Amount', alignRight: false },
  { id: 'totalMonths', label: 'Total Months', alignRight: false },
...
export default function UserPage() {
  ...
  const buyer = useSelector((s) => s?.buyer?.data?.data);
  console.log('___buyer___', buyer);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = buyer.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - buyer?.length) : 0;

  const filteredUsers = applySortFilter(buyer, getComparator(order, orderBy), filterName);
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill" />} onClick={() => navigate('newplot')}>
            New Buyer
          </Button>
        </Stack>
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">
                          <Typography align="left">{ownerName}</Typography>
                        </TableCell>
                        ...
                        <TableCell align="left">{monthlyInstallment}</TableCell>
                        <TableCell align="left">{startDate}</TableCell>
                        <TableCell align="left">{totalAmount}</TableCell>
                        <TableCell align="left">{totalMonths}</TableCell>
